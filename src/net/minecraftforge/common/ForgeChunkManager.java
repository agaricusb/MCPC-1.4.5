...
import mcpc.com.google.common.cache.CacheBuilder;
...
import mcpc.com.google.common.collect.ImmutableSet;
...
public class ForgeChunkManager
{
    ...
    public static class Ticket
    {
        ...
        Ticket(String modId, Type type, World world, EntityPlayer player)
        {
            ...
            if (player != null)
            {
                this.player = player.getLocalizedName();
            }
            else
            {
    ...
    static void loadWorld(World world)
    {
        ...
        if (chunkLoaderData.exists() && chunkLoaderData.isFile())
        {
            ...
            try
            {
                forcedChunkData = NBTCompressedStreamTools.a(chunkLoaderData);
            }
            catch (IOException e)
            {
            ...
            NBTTagList ticketList = forcedChunkData.getList("TicketList");
            for (int i = 0; i < ticketList.size(); i++)
            {
                NBTTagCompound ticketHolder = (NBTTagCompound) ticketList.get(i);
                ...
                NBTTagList tickets = ticketHolder.getList("Tickets");
                for (int j = 0; j < tickets.size(); j++)
                {
                    NBTTagCompound ticket = (NBTTagCompound) tickets.get(j);
            ...
            for (Ticket tick : ImmutableSet.copyOf(pendingEntities.values()))
            {
                if (tick.ticketType == Type.ENTITY && tick.entity == null)
                {
                    ...
                    // to the ticket
                    world.getChunkAt(tick.entityChunkX, tick.entityChunkZ);
                }
            }
            for (Ticket tick : ImmutableSet.copyOf(pendingEntities.values()))
            {
    ...
    public static Ticket requestPlayerTicket(Object mod, EntityPlayer player, World world, Type type)
    {
        ...
        if (playerTickets.get(player.getLocalizedName()).size()>playerTicketLength)
        {
            FMLLog.warning("Unable to assign further chunkloading tickets to player %s (on behalf of mod %s)", player.getLocalizedName(), mc.getModId());
            return null;
        }
        Ticket ticket = new Ticket(mc.getModId(),type,world,player);
        playerTickets.put(player.getLocalizedName(), ticket);
    ...
    static void saveWorld(World world)
    {
        ...
        forcedChunkData.set("TicketList", ticketList);
        ...
        for (String modId : ticketSet.keySet())
        {
            ...
            ticketHolder.set("Tickets", tickets);
            ...
            for (Ticket tick : ticketSet.get(modId))
            {
                ...
                {
                    ticket.setCompound("ModData", tick.modData);
                }
                if (tick.ticketType == Type.ENTITY && tick.entity != null && tick.entity.c(new NBTTagCompound()))
                {
                    ticket.setInt("chunkX", MathHelper.floor(tick.entity.ai));
                    ticket.setInt("chunkZ", MathHelper.floor(tick.entity.ak));
        ...
        try
        {
            NBTCompressedStreamTools.b(forcedChunkData, chunkLoaderData);
        }
        catch (IOException e)
        {
