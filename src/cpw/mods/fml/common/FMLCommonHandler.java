...
public class FMLCommonHandler
{
    ...
    private Set<WorldNBTStorage> handlerSet = Sets.newSetFromMap(new MapMaker().weakKeys().<WorldNBTStorage,Boolean>makeMap());
    ...
    public Side getEffectiveSide()
    {
        ...
        if ((thr instanceof ThreadServerApplication) || (thr instanceof DedicatedServerConnectionThread))
        {
            return Side.SERVER;
        }
    ...
    public void onPlayerPreTick(EntityHuman player)
    {
        Side side = player instanceof EntityPlayer ? Side.SERVER : Side.CLIENT;
        tickStart(EnumSet.of(TickType.PLAYER), side, player);
    }

    public void onPlayerPostTick(EntityHuman player)
    {
        Side side = player instanceof EntityPlayer ? Side.SERVER : Side.CLIENT;
    ...
    public void handleWorldDataSave(WorldNBTStorage handler, WorldData WorldData, NBTTagCompound tagCompound)
    {
        ...
        while (iterator.hasNext())
        {
            ...
            if (mc instanceof InjectedModContainer)
            {
                ...
                if (wac != null)
                {
                    NBTTagCompound dataForWriting = wac.getDataForWriting(handler, WorldData);
                    tagCompound.setCompound(mc.getModId(), dataForWriting);
                }
            }
        }
    }

    public void handleWorldDataLoad(WorldNBTStorage handler, WorldData WorldData, NBTTagCompound tagCompound)
    {
        ...
        while (iterator.hasNext())
        {
            ...
            if (mc instanceof InjectedModContainer)
            {
                ...
                if (wac != null)
                {
                    wac.readData(handler, WorldData, additionalProperties, tagCompound.getCompound(mc.getModId()));
                }
            }
        }
    }
